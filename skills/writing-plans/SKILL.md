...
**Announce at start:** "I'm using `devstack:writing-plans` to create the implementation plan."

**Context:** This runs in a worktree already created by `devstack:brainstorming` (via `devstack:using-git-worktrees`). If not in a worktree, set one up first.
...
> **For agentic workers:** REQUIRED SUB-SKILL: Use `devstack:subagent-driven-development` (recommended) or `devstack:executing-plans` to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax.
...
This information feeds into `devstack:dispatching-parallel-agents` during execution.
...
**If Subagent-Driven:**
- REQUIRED SUB-SKILL: `devstack:subagent-driven-development`
- Fresh subagent per task + two-stage review

**If Inline:**
- REQUIRED SUB-SKILL: `devstack:executing-plans`
