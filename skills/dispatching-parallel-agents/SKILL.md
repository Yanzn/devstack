...
**When to use:** the user has chosen a deepening candidate from `devstack:improving-architecture` and wants to explore alternative interfaces before committing. Also useful for: API redesigns, new module shapes, anything where the seam placement is itself the question.

**Process:**

1. **Frame the problem space** for the user — constraints, dependencies (and their category from `devstack:improving-architecture/DEEPENING.md`), an illustrative code sketch. Show this; the user reads while agents work.
...
The full prompt brief, including the architecture vocabulary every agent must use, lives in `devstack:improving-architecture/INTERFACE-DESIGN.md`.
